...
if($sql){echo '设置成功！</div></div>';}
else{echo '设置失败！</div></div>';}
}else{echo "新密码不能为空，也不可以和之前密码相同";}
echo '<hr/><a href="./daili.php">>>返回代理列表</a>      </div>
      </div>';
foot();
exit;
}elseif($my=='qq'){
...
echo '<div class="col-md-12">
      <div class="panel panel-default">
       ...
       <form action="./daili.php?" method="get" class="form-inline" role="form">
            ...
            <div class="form-group">
            <input type="text" name="user" value='.$user.' hidden />
            <input type="text" name="my" value="qq" hidden/>
          <input type="text" name="qq" value='.$rs['qq'].' />
            </div>
            <input type="submit" value="更改新Q Q" class="btn btn-primary"/>
          </form>';
$qq=$_GET['qq'];
if($qq<>$rs['qq'] && $qq<>""){
$sql=$DB->query("update `auth_daili` set `qq`='$qq' where `user`='{$user}'");
if($sql){echo '设置成功！</div></div>';}
else{echo '设置失败！</div></div>';}
}else{echo "新Q Q不能为空";}
echo '<hr/><a href="./daili.php">>>返回代理列表</a>      </div>
...
if($sql){echo '设置成功，状态为已激活！</div></div>';}
else{echo '设置失败！';}
echo '<hr/><a href="./daili.php">>>返回代理列表</a>      </div>
      </div>';
foot();
...
if($_GET['vip'] && $_GET['user']){
	...
	if($sql){echo '升级成功，当前为VIP'.$vip.'！</div></div>';}
else{echo '升级失败！</div></div>';}
echo '<hr/><a href="./daili.php">>>返回代理列表</a></div></div>';
}else{echo '请填写！</div></div>';}
foot();
exit;
}elseif($my=="del"){
	$user=$_GET['user'];
...
if($sql){echo '删除代理'.$user.'成功！</div></div>';}
else{echo '删除失败！</div></div>';}
echo '<hr/><a href="./daili.php">>>返回代理列表</a></div></div>      ';
foot();
exit;
}
...
<div class="col-md-12">
      <div class="panel panel-default">
        ...
        <table class="table table-striped">
          <thead><tr><th>用户名</th><th>状态</th><th>金币</th><th>密码</th><th>代理级别</th><th>QQ</th><th>操作</th></tr></thead>
          <tbody>
<?php
if(isset($_GET['sdl'])) {
	$sdl=$_GET['sdl'];
	...
	if($_GET['type']==1) {
		...
		$sql=" `qq`='{$_GET['sdl']}'";
		$numrows=$DB->count("SELECT count(*) from auth_daili WHERE{$sql}");
		$con='包含 '.$_GET['qq'].' 的共有 <b>'.$numrows.'</b> 个信息';
	}
}else{
...
echo '<tr><td><b>'.$res['user'].'</b></td><td>'.$isactive.'</td><td>'.$res['rmb'].'</td><td>'.$res['pass'].'</td><td>'.$v.'</td><td>'.$res['qq'].'</td><td>'.($res['active']==1?'<a href="./daili.php?my=black1&user='.$res['user'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要更改状态吗？\');">设为未激活</a>':'<a href="./daili.php?my=black0&user='.$res['user'].'" class="btn btn-xs btn-success" onclick="return confirm(\'你确实要将此代理激活吗？\');">设置为激活</a>').'<a href="./daili.php?my=black2&user='.$res['user'].'" class="btn btn-xs btn-primary">为其充值</a><a href="./daili.php?my=qq&user='.$res['user'].'" class="btn btn-xs btn-default">更改Q Q</a><a href="./daili.php?my=black3&user='.$res['user'].'" class="btn btn-xs btn-info">改密码</a><a href="./daili.php?my=vip&user='.$res['user'].'" class="btn btn-xs btn-success">设置VIP等级</a><a href="./daili.php?my=del&user='.$res['user'].'" class="btn btn-xs btn-warning" onclick="return confirm(\'你确实要进行操作吗？\');">删除代理</a></td></tr>';
}
?>
          </tbody>
        </table>
