...
echo "<ul class='list-group'><li class='list-group-item'>成功生成以下卡密</li>";
for ($i = 0; $i < $num; $i++) {
	$km=getkm(18);
...
echo '</ul><hr><a href="./kmlist.php" class="btn btn-default btn-block">>>返回卡密列表</a></div></div>';
foot();
}
...
echo '<form action="kmlist.php?my=add" method="POST" class="form-inline">
  ...
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="每个开通的月数">
  </div>
...
if(isset($_GET['kw'])) {
	...
	$sql=" kind=1";
	$con='代挂平台共有 <b>'.$numrows.'</b> 个卡密';
}
echo $con;
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>卡密</th><th>状态</th><th>生成代理</th><th>添加时间</th><th>使用时间</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
{
$res['daili']?$dailis = $DB->get_row("select * from auth_daili where id='{$res['daili']}' limit 1"):"管理员";
$daili = $dailis['user']?$dailis['user']:"管理员";
$usetime = $res['usetime']?$res['usetime']:"未使用";
if($res['isuse']==1) {
	$isuse='<font color="red">已使用</font><br/>使用者:'.$res['user'];
} elseif($res['isuse']==0) {
	$isuse='<font color="green">未使用</font>';
}
echo '<tr><td><b>'.$res['km'].'</b></td><td>'.$isuse.'</td><td>'.$daili.'</td><td>'.$res['addtime'].'</td><td>'.$usetime.'</td><td><a href="./kmlist.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此卡密吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
