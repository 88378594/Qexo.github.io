...
echo '</ul><hr><a href="./dlkm.php" class="btn btn-default">>>返回卡密列表</a></div></div>';
foot();
}
...
if(isset($_GET['kw'])) {
	...
	$sql=" kind=2";
	$con='代挂平台共有 <b>'.$numrows.'</b> 个代理余额卡密';
}
echo $con;
?>
...
echo '<tr><td><b>'.$res['km'].'</b></td><td>'.$isuse.'</td><td>'.$res['addtime'].'</td><td>'.$res['usetime'].'</td><td><a href="./dlkm.php?my=del&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此卡密吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
